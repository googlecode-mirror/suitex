...
class Calendar{
	...
	function output_calendar($year = NULL, $month = NULL, $calendar_class = 'calendar', $inst = 0){
		...
		for($day=1,$cell=$prepend+1; $day<=$days_in_month; $day++,$cell++){
			...
			*/
			...
			$output .= "\t<td" . $day_class . " title=\"" . ucwords(strftime($title_format, strtotime($day_date))) . "\" id=\"cell-" . $inst . '-' . number_format($day, 0) . "\">";
			...
			switch( $this->link_days ){
				...
				case 1 :
					if( empty($this->formatted_link_to) ){
						...
						$output .= "<a href=\"" . strtr(strftime($this->formatted_link_to, strtotime($day_date)), array('instance' => $inst, 'jcounter' => number_format($day, 0))) . "\">" . $day . "</a>";
					}
				break;
